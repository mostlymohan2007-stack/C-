@echo off
echo Compiling Space Battle Game with Simple User Upload Support...
echo.

REM Set SFML path
set SFML_PATH=C:\SFML-2.6.1-windows-vc17-64-bit\SFML-2.6.1

REM Compile with Visual Studio using original files with upload support
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo 1. Copy your custom images to game folder
    echo 2. Use recommended sizes for best results
    echo 3. Restart game to load new images
    echo 4. Game falls back to colored shapes if no images found
    echo.
) else (
    echo.
